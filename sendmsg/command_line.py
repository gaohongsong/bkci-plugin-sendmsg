...
def exit_with_error(error_type=None, error_code=None, error_msg="failed"):
    ...
    sdk.log.error(
        "error_type: {}, error_code: {}, error_msg: {}".format(
...
def main():  # noqa
    ...
    if "mail" in send_by and mail_receiver:
        ...
        resp_json = get_response(
            url=list_user_url, method="post", data=request_data, desc="list_users"
        )
    ...
    if send_by_dingbot_str == "true":
        ...
        if dingbot_msgtype == "text":
            ...
            sdk.log.info(
                "dingbot_at_username_list is {}".format(dingbot_at_username_list)
            )
    ...
    if "weixin" in send_by:
        ...
        sdk.log.info("【weixin】 response is {}".format(resp_json))
        ...
        if attachment_file:
            ...
            if not os.path.exists(file_path):
                ...
                with open(file_path, "rb") as f:
                    ...
                    item = {"filename": attachment_file, "content": content_encoded}
                    attachments.append(item)
                mail_data_tpl.update({"attachments": attachments})

        resp_json = get_response(
            url=mail_url, method="post", data=mail_data_tpl, desc="mail"
        )
        sdk.log.info("【mail】 response is {}".format(resp_json))

    if "dingtalk" in send_by:
        msg_tpl["msg_type"] = "ding"
        resp_json = get_response(
            url=msg_url, method="post", data=msg_tpl, desc="dingtalk"
        )
        sdk.log.info("【dingtalk】 response is {}".format(resp_json))
    ...
    if send_by_webot:
        ...
        if artifact:
            ...
            if not os.path.exists(file_path):
                ...
                media_body = {"msgtype": "file", "file": {"media_id": media_id}}
                r = requests.post(
                    webhook_url, headers=headers, data=json.dumps(media_body)
                )
                r_json = r.json()
                if r.status_code != 200 or r_json["errcode"] != 0:
    ...
    if send_by_dingbot:
        ...
        resp_json = get_response(
            url=dingbot_url, method="post", data=dingbot_data_tpl, desc="dingbot"
        )
        sdk.log.info("【dingbot】 response is {}".format(resp_json))
